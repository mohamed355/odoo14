...
odoo.define('pos_order_return_reprint.pos_order_return', function(require) {
    "use strict";
    ...
    order_model.fields.push('barcode','return_order_id', 'payment_ids', 'is_return_order', 'return_status', 'amount_total');
          ...
          render_list(order, input_txt){
                  ...
                  if (input_txt != undefined && input_txt != '') {
                      ...
                      for (var i = 0; i < order.length; i++) {
                          ...
                          if (((order[i].name.toLowerCase()).indexOf(search_text) != -1)
                              || ((order[i].partner_id[1].toLowerCase()).indexOf(search_text) != -1)
                              ||((order[i].pos_reference.toLowerCase()).indexOf(search_text) != -1)
                              ||((order[i].barcode.toLowerCase()).indexOf(search_text) != -1)) {
                              new_order_data = new_order_data.concat(order[i]);
                          }
                      }
                      order = new_order_data;
