...
class PosDashboard(models.Model):
    ...
    def set_total_amount_ref(self,total):
        magnitude = 0
        while abs(total) >= 1000:
            magnitude += 1
            total /= 1000.0
        # add more suffixes if you need them
        val_total = '%.2f%s' % (total, ['', 'K', 'M', 'G', 'T', 'P'][magnitude])
        return val_total
    @api.model
    def get_refund_details(self,option):
        default_date = datetime.today().date()
        pos_order = self.env['pos.order'].search([])

        total = sum([x.amount_total for x in pos_order])
        total_refund_count = len(list(filter(lambda x: x.amount_total < 0.0 , pos_order)))
        total_order_count = len(pos_order)
...
# yesterday = today - datetime.timedelta(days=1)
        # pos_order_today = self.env['pos.order'].search([('date_order','=',default_date)])
        # pos_order_today = self.env['pos.order'].browse(filter(lambda x: x.date_order.date()==default_date , pos_order))
        pos_order_today = list(filter(lambda x: x.date_order.date()==default_date , pos_order))

        if not option or option == 'pos_today_order':
            pos_order_today = list(filter(lambda x: x.date_order.date()==default_date , pos_order))
            total_session = len(list(filter(lambda x: x.start_at.date()==default_date , pos_session)))
        elif option == 'pos_10Days_order':
            default_date = default_date - timedelta(days=10)
            pos_order_today = list(filter(lambda x: x.date_order.date()>=default_date , pos_order))
            total_session = len(list(filter(lambda x: x.start_at.date()>=default_date , pos_session)))

        elif option == 'pos_this_month_sales':
            default_date = default_date.replace(day=1)
            pos_order_today = list(filter(lambda x: x.date_order.date()>=default_date , pos_order))
            total_session = len(list(filter(lambda x: x.start_at.date()>=default_date , pos_session)))

        elif option == 'pos_last_month_sales':
            first_month = default_date.replace(day=1)- timedelta(days=1)
            first_month = first_month.replace(day=1)
            last_month = default_date.replace(day=1) - timedelta(days=1)
            pos_order_today = list(filter(lambda x: x.date_order.date()<= last_month and x.date_order.date()>=first_month , pos_order))
            total_session = len(list(filter(lambda x: x.start_at.date()<= last_month and x.start_at.date()>=first_month , pos_session)))

        elif option == 'pos_this_year_sales':
            default_date = date(default_date.year, 1, 1)
            pos_order_today = list(filter(lambda x: x.date_order.date()>=default_date , pos_order))
            total_session = len(list(filter(lambda x: x.start_at.date()>=default_date , pos_session)))

        elif option == 'pos_last_year_sales':
            first = date(default_date.year, 1, 1)
            last = first - timedelta(days=1)
            first = date(last.year, 1, 1)
            pos_order_today = list(filter(lambda x: x.date_order.date()<= last and x.date_order.date()>=first , pos_order))
            total_session = len(list(filter(lambda x: x.start_at.date()<= last and x.start_at.date()>=first , pos_session)))


# last = date(date.today().year, 12, 31)

        pos_order_today_refund = list(filter(lambda x: x.amount_total < 0.0 , pos_order_today))
        pos_order_today = list(filter(lambda x: x.amount_total > 0.0 , pos_order_today))


        today_refund_total = len(pos_order_today_refund)
        today_sale = len(pos_order_today)
        total_order_amount = sum([x.amount_total for x in pos_order_today])
        total_order_refund_amount = sum([x.amount_total for x in pos_order_today_refund])
        ...
        # print({
        #     'total_sale': self.set_total_amount_ref(total),
        #     'total_order_count': total_order_count,
        #     'total_refund_count': total_refund_count,
        #     'total_session': total_session,
        #     'today_refund_total': today_refund_total,
        #     'today_sale': today_sale,
        #     'total_order_amount': self.set_total_amount_ref(total_order_amount),
        #     'total_order_refund_amount': self.set_total_amount_ref(total_order_refund_amount),
        # })
