...
class PosOrder(models.Model):
	...
	commission_ids = fields.One2many('invoice.sale.commission', 'pos_order_id', string='POS Commissions')
	...
	def get_mix_commission(self, commission_brw, order):
		...
		for line in order.lines:
			...
			margin = ((line.price_subtotal / line.qty) - line.product_id.standard_price)
	...
	def get_partner_commission(self, commission_brw, order):
		...
		for line in order.lines:
			amount = line.price_subtotal
			invoice_commission_data = {}
			if (order.partner_id.user_id and order.partner_id.user_id.id in sales_person_list) and order.partner_id.is_affiliated == True:
				...
				invoice_commission_data = {'name' : name,
								   ...
								   'pos_order_id' : order.id,
								   'date':datetime.datetime.today()}
			elif (order.partner_id.user_id and order.partner_id.user_id.id in sales_person_list) and  order.partner_id.is_affiliated == False:
